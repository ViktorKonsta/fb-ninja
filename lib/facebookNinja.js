...
const facebookNinja = co.wrap(function*(data, options = {}) {
  ...
  if (!data) {
    throw new Error("First argument expected but got nothing")
  }

  if (!isArray(data) && !isPlainObject(data)) {
    throw new Error("First argument can be array or object")
  }

  if (isArray(data) && !data.length) {
    throw new Error("First argument must have object within array")
  }

  // for basic benchmarking (see "stat" within final return)
  const a = +new Date()

  // destructuring a lot of options
  const {
  ...
  const requests = chunks.map(chunk => {
    ...
    request.body.batch = chunk.map(doc => {
      ...
      if (doc.method.match(/POST|PUT|DELETE/g)) {
        batch.body = ""
  ...
  for (let chunkIndex = 0; chunkIndex < requests.length; chunkIndex++) {
    ...
    batchResponses.forEach((responseBody, responseIndex) => {

      // get request in original form (given by developer)
      const relatedDoc = chunks[chunkIndex][responseIndex]

      // and assign it to a response
      const response = Object.assign({}, responseBody, { request: relatedDoc })
